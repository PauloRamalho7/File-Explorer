...
begin
    arquivo := '';
    caminho := System.IOUtils.TPath.GetSharedDocumentsPath;

    MontarLista(caminho, true);
end;

procedure TFrmExplorer.AddListItem(list: array of string; itype: string);
var
    c: integer;
    LItem: TListBoxItem;
    BitmapFolder, BitmapFile: TBitmap;
begin
    BitmapFolder := FrmExplorer.img_folder.Bitmap;
    BitmapFile := FrmExplorer.img_file.Bitmap;
...
procedure TFrmExplorer.MontarLista(path_tr: string; clear: boolean);
var
...
procedure TFrmExplorer.btn_cad_voltarClick(Sender: TObject);
begin
    // Fecha a tela...
...
procedure TFrmExplorer.btn_fecharClick(Sender: TObject);
begin
    arquivo := '';
    close;
end;

procedure TFrmExplorer.FormKeyUp(Sender: TObject; var Key: Word;
...
procedure TFrmExplorer.ListBoxItemClick(const Sender: TCustomListBox;
  const Item: TListBoxItem);
begin
